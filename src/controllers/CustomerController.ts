...
export const CreateOrder = async (
    ...
    if (customer) {
        ...
        // Create a lookup map for cart items by their _id
        const cartLookup = new Map(cart.map(({ _id, unit }) => [_id, unit]));

        // Loop through foods and check against the cart lookup map
        foods.forEach((food) => {
            if (cartLookup.has(food._id)) {
                const unit = cartLookup.get(food._id);
